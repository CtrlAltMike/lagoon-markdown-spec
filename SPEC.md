...
Readers MUST ignore unrecognized safe files. A reader that rewrites a package
SHOULD preserve them byte-for-byte unless the user deliberately removes them.
...
After NFC normalization, every path MUST be unique. Readers MUST reject an
archive containing duplicate normalized paths. Matching of manifest paths to
archive entries is case-sensitive.
...
structural requirements. The grapheme-cluster length limits require an
additional Unicode-aware check.
...
Files below `assets/` MAY be PNG, JPEG, GIF, WebP, or passive SVG. The filename
extension and encoded type MUST agree. Asset paths may be referenced from
`document.md` with ordinary relative Markdown URLs, for example:
